...
const select_box = document.querySelectorAll(".select-box .options");
const search_box = document.querySelectorAll(".search-box");
const input_box = document.querySelectorAll('input[type="tel"]');
const selected_option = document.querySelectorAll(".selected-option div");
let options = null;
  ...
  const option = `
    <li class="option">
      <div>
        ...
        <span class="country-code" style="opacity:0">${country.code}</span>
      </div>
      <strong>+${country.phone}</strong>
    </li>
  `;
  select_box.forEach((el) => {
    el.querySelector("ol").insertAdjacentHTML("beforeend", option);
  });
}
options = document.querySelectorAll(".option");
selected_option.forEach((el, i) => {
  el.addEventListener("click", () => {
    document.querySelector(`#options-${i + 1}`).classList.toggle("active");
  });
});

options.forEach((el, i) => {
  el.addEventListener("click", function () {
    const id = [...select_box].indexOf(this.closest(".options"));
    selectOption.call(this, id);
  });
});

function selectOption(id) {
  const phone_code = this.querySelector("strong").cloneNode(true);
  const country_code = this.querySelector(".country-code").innerText;
  selected_option[id].innerHTML = "";
  selected_option[id].append(phone_code);
  input_box[id].value = phone_code.innerText;
  select_box[id].classList.remove("active");
  selected_option[id].classList.remove("active");
  search_box[id].value = "";
  select_box[id]
    .querySelectorAll(".hide")
    .forEach((el) => el.classList.remove("hide"));
  document.querySelector("#country").value = country_code;
}

//====== Get User Location =======
async function getLocation() {
  await fetch(
    "https://api.ipgeolocation.io/ipgeo?apiKey=cd5e6560415b48c9a74ba7a636ef4bcd"
  )
    .then((res) => res.json())
    .then((res) => {
      sessionStorage.setItem("country_code", res.country_code3);
      document.querySelectorAll(".callcode").forEach((el) => {
        el.innerText = res.calling_code;
      });
    });
}
getLocation();
...
//====== Search Country Name =======
search_box.forEach((search, index) => {
  search.addEventListener("input", function () {
    const filter = search.value.toLowerCase();
    const options = select_box[index].querySelectorAll(".option");
